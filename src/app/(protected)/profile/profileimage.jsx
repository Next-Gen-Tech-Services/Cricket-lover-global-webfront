...
export default function ProfileImageUpload() {
  ...
  const handleUpload = async () => {
    if (!image) return toast("Select image first");
    ...
    <div className="flex items-center gap-6">
      <div className="relative w-28 h-28">
        {preview || userData?.avatarUrl ? (
          <img
            src={preview || userData.avatarUrl}
        ...
        <button
          ...
          className="absolute bottom-0 right-0 bg-[#00a63e] p-2 rounded-full text-white"
        >
          <Plus size={16} />
        </button>
      ...
      <button
        ...
        disabled={!image}
        onClick={handleUpload}
        className="bg-[#00a63e] text-white px-5 py-2 rounded-lg disabled:opacity-50"
      >
        Save Profile Image
      </button>
    </div>
